...
- The binder's folder order marker is tilde-delimited (`020~ Part One`). The tilde makes plugin-written markers unmistakable, so a folder name that happens to start with a number — `2023 files` — is always treated as a plain name: displayed in full, sorted naturally, and never renamed by a reorder. Reordering a folder rewrites only the marker; the name as typed stays byte-identical behind it. (#239)
- Project templates scaffold real structure: the book template's part is a real folder, documents carry their order, status, and compile settings in frontmatter (`binder-order`, `binder-status`, `binder-compile`, `word-count-goal`), and a document's filename is its title. (#233)
- The launcher's project row is identical to the binder's: the same project dropdown (now carrying a project-switch chevron on both surfaces) with the same boxed "+", pencil, and trash buttons grouped at its right edge. It replaces the launcher's separate project-name line, icon-menu switcher, and choose-a-project dropdown. (#233 audit)

### Removed
- The "Scan project folder for new files" command and binder scan button, the "Add to writing project" file-menu action and its modal, group/part structural items, "Remove from binder", and the "Include research notes" export option — all artifacts of the retired `_binder.json` model. Membership is location now: adding a file to a project is moving it into the project folder, Research never compiles (the zone boundary is the compile boundary), and folders are the structure. The per-document word count goal similarly has one home: the `word-count-goal` frontmatter key, read and written by every surface. (#233)

### Added
- Export title choice: the export dialog gains a Title dropdown that names the export everywhere at once — the filename, the title page, and the file metadata (epub title, cover, and table of contents; HTML and manuscript page titles) are always identical. A folder export offers "Folder name" (the default — the file opens with only the folder's display name), "Project — folder", "Project name", and "Type your own title"; other exports offer the project name and a custom title. Choosing a custom title reveals a field for typing it, and Export waits until one is entered. "Preview compiled manuscript" now renders exactly what the export will produce — the same documents, headings, title page, and title as the dialog's current selections, including folder exports and the folder-names-as-headings option — and proceeding to export from the preview keeps those selections. (#243, #244, #260)
- Silent migration: a project whose legacy `_binder.json` still differs from its files migrates automatically on activation — no prompt, no preview, no user action. Legacy groups and parts become real folders (created with order markers, or matched to existing folders by name; a matched plain folder gets its marker attached in place, one that already carries a marker keeps it), documents move into them **keeping their filenames** — nothing is ever renamed to a stored title — and per-document metadata lands in frontmatter (`binder-order`, `binder-status`, `binder-type`, `word-count-goal`, and `binder-compile: false` for legacy compile exclusions), writing only keys not already set: values you set yourself always win, re-checked at the moment of writing. When two documents would need the same name in one folder, the later one (in legacy order) simply stays where it is and is ordered there — a file's name is never silently changed and nothing is ever displaced. A folder the old binder recorded documents inside — invisible to the old format as an item — receives an order marker placing it just before its earliest recorded document, so it never sinks below the documents it used to hold; a folder alone in its group is never renamed for ordering it doesn't need. Every operation is independently atomic and the pass is resumable: an interruption is recovered by simply re-running, which happens on the next project activation. Failures are tolerated quietly (sync locks are routine); only an operation that keeps failing produces a single plain-language notice, once. Migration can create, move, and rename — it never deletes anything, no code path can alter document prose, and `_binder.json` is never modified. (#230, #231, ADR 0001 slices 6–7)
- Restore previous binder layout: a command that undoes the migration's **layout** — documents return to their pre-migration locations (their old folders are recreated if needed), order markers come off matched folders, and folders the migration created remain behind empty rather than risk deleting anything. Computed from the preserved `_binder.json`, so it needs no journal; a document you have since moved or renamed is skipped and reported, never guessed at. This is a layout restore, not a full revert — frontmatter written by migration stays (it is indistinguishable from values you set yourself). After a restore, downgrading the plugin finds the vault laid out the way the classic binder expects. A restore also sticks: the choice is recorded in the project itself (it travels with the vault and survives reinstalls), so a restored project is never automatically rearranged again. (#231, #233)
- Compile and export from the manuscript zone: "Entire project" export and the compile preview read the project folder tree directly — every manuscript-zone document, depth-first in binder order — instead of the retired `_binder.json` records (whose stale paths previously made whole-project export silently drop documents). The zone boundary is the compile boundary: Research and Exports never compile. Documents marked `binder-compile: false` are excluded from the preview and every format. Document titles come from the filename. A new default-off export option, "Include folder names as headings", inserts each folder's display name as a heading at its depth and shifts document headings one level below (off, output is unchanged — documents stay flat `#` headings). Right-clicking a manuscript folder offers "Export folder": the same export dialog scoped to that folder's subtree, heading depths measured from that folder and the export titled after the folder by default (see the export title choice above). Epub chapters follow the same order and exclusions (folder headings do not apply to epub). (#232, ADR 0001 slice 8)
- Context menu and metadata editing in the binder: right-click any row (F2 renames, Shift+F10 opens the menu from the keyboard) to rename, set or clear status (`binder-status`, shown live as the left-edge stripe), set a word count goal, set or clear an optional document type (`binder-type`, shown in the hover tooltip), exclude a document from compile (`binder-compile: false`, row dims; re-including removes the key), create a new document or folder, or delete to the trash (respecting the app's "Deleted files" setting, with a confirm that states a folder's contained file count). Renames go through one atomic operation so links heal; a folder keeps its order marker, a document keeps its `binder-order`, and a file's extension is never editable from the binder. Invalid names — forbidden characters, trailing space or period, or a name already taken — are rejected with a specific message, never silently altered. New items are created without an order value and sort naturally; ordering starts with the first drag. Toolbar buttons create documents and folders at the manuscript root; a folder's menu creates inside it, a document's menu creates beside it. Research rows offer only rename, create, and delete; Exports rows only delete; non-markdown files only rename and delete. The frontmatter `word-count-goal` key is the sole goal authority — the goal modal, the inline goal banner, the status bar, and the targets dashboard (which lists the manuscript zone) all read and write it directly. (#229, ADR 0001 slice 5)
- Structural moves in the binder: dragging physically moves files and folders. Dropping between rows of any visible group moves the item there (one atomic rename, links auto-heal); dropping onto the middle of a folder row nests the item at the end of that folder (collapsed folders expand after a short hover); dropping on the empty space below the manuscript promotes to the project root; a folder drag carries all its children. Markdown documents also move both ways between the manuscript and Research — drop onto the Research drawer tab (hovering it opens the drawer) or a folder inside it; Research never receives ordering writes, so an untouched Research folder stays naturally sorted. Drops into Exports are refused with an explanation (it is output-only), folders cannot cross zones, a folder cannot be dropped into itself, nesting under a document is never offered, and non-markdown files never drag. A failed move (for example, a name collision at the destination) is reported and leaves nothing half-moved. (#228, ADR 0001 slice 4)
- Drag-to-reorder in the binder (manuscript zone, among siblings): dropping a document writes its position as an integer `binder-order` frontmatter key; dropping a folder renames its tilde-delimited name marker (links auto-heal, the displayed name never changes). Writes are lazy and minimal — a single midpoint value when a gap exists, `next − 10` at the front, `prev + 10` at the back; only when a group has no usable gap (first reorder of a never-ordered group, exhausted gap, duplicate values) does that one sibling group renumber as 10/20/30. Document filenames are never touched for ordering, and non-markdown files never receive writes. Drop position shows as an accent line between rows. (#227, ADR 0001 slice 3)
- Resources drawer: Research and Exports pinned below the manuscript tree as drawer tabs with live file counts (every visible file in the folder, not just markdown). Clicking a tab opens that zone's tree — markdown and viewable files open in a tab, anything the app cannot display opens in the system default app — and clicking the open tab collapses the drawer. Open state and last-open tab persist per project as a view preference. Empty zones show quiet placeholder text; a missing Research or Exports folder simply shows a count of zero (nothing is created). (#226, ADR 0001 slice 2)
- The redesigned binder view: renders the active project's folder tree directly instead of `_binder.json`. Documents order by an integer `binder-order` frontmatter key and folders by a tilde-delimited name marker (`020~ Part One`, stripped in display) on one shared number line; anything without a value sorts naturally at the end of its group. Files created, renamed, or deleted outside the app — including in Windows Explorer — appear correctly with no scan. Rows show status as a left-edge stripe (from `binder-status`), documents excluded from compile (`binder-compile: false`) and non-markdown files render dimmed, folders carry document-count badges behind a toggle, and hovering shows the on-disk name plus the frontmatter the binder reads. (#225, ADR 0001 slice 1)
