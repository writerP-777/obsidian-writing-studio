...
### Added
- Carry-over preview (dry run) for the experimental filesystem binder: a project whose legacy `_binder.json` still differs from its files now offers a one-time notice (Preview / Not now — re-offered any time via the "Preview binder carry-over" command or the binder's project-row button) opening a preview that lists every operation carrying the legacy binder over would perform — folders to create for legacy groups/parts, each document's single move-and-rename to its binder title, and the frontmatter to write (`binder-order`, `binder-status`, `binder-type`, `word-count-goal`, and `binder-compile: false` for legacy compile exclusions). Frontmatter you have already set always wins — carry-over only fills in what's absent, and a folder that already carries an order marker keeps it. The plan is deterministic: duplicate titles get numbered suffixes assigned by legacy order, Windows-reserved names (`CON`, `PRN`, `AUX`, `NUL`, `COM1`–`9`, `LPT1`–`9`) are suffixed past the reservation, and a file found at neither its original nor its new location — or whose new location is already taken — is flagged for attention and left untouched. Nothing executes in this slice: the preview is provably read-only and `_binder.json` is never modified. (#230, ADR 0001 slice 6)
