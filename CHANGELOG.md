...
### Added
- Export folder title setting (experimental filesystem binder): a folder export's title page and metadata can name the exported folder or the whole project — Settings → Export → "Export folder title", default folder name. With the default, exporting a folder opens with the folder's display name as the title and keeps the project title as a line beneath it, and the epub title, cover, and manuscript title page follow suit, so a folder export can no longer be mistaken for a whole-project export. Choosing "Project title" restores the previous behavior exactly. Entire-project exports are unaffected either way. (#244)
