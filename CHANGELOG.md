...
### Fixed
- Selected states in the launcher are now legible in light themes. The selected writing-mode button, an active Focus/Typography toggle, the launcher's call-to-action buttons, and the active settings tab hardcoded white text on the accent fill, which washed out in Obsidian's light theme (and any theme that pairs its accent with dark text). They now use the theme's `--text-on-accent` token — the same approach the binder control strip already uses — so they read correctly in every theme, light or dark. (#191)
