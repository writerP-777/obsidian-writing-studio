...
### Added
- Drag-to-reorder in the experimental filesystem binder (manuscript zone, among siblings): dropping a document writes its position as an integer `binder-order` frontmatter key; dropping a folder renames its numeric name prefix (links auto-heal, the displayed name never changes). Writes are lazy and minimal — a single midpoint value when a gap exists, `next − 10` at the front, `prev + 10` at the back; only when a group has no usable gap (first reorder of a never-ordered group, exhausted gap, duplicate values) does that one sibling group renumber as 10/20/30. Document filenames are never touched for ordering, and non-markdown files never receive writes. Drop position shows as an accent line between rows; drops outside the sibling group are not accepted in this slice (moving into folders and across zones comes later). (#227, ADR 0001 slice 3)
