...
// The manuscript zone owns compile (#232/#233): the zone lives in a mock
// vault folder tree + metadata cache, document contents in the in-memory
// adapter.
...
describe('ExportEngine.compileContent basics', () => {
  it('strips frontmatter when includeFrontmatter is false, keeps it when true', async () => {
    const { engine } = await makeFsWorld(true);

    const without = await engine.compileContent(projectOpts());
    const withFm = await engine.compileContent(projectOpts({ includeFrontmatter: true }));

    expect(without).not.toContain('word-count');
    expect(without).not.toContain('title: "One"');
    expect(withFm).toContain('word-count: 9');
  });

  it('replaces the in-document h1 with the canonical title heading, not duplicating it', async () => {
    const { engine } = await makeFsWorld(true);

    const compiled = engine.toMarkdown(await engine.compileContent(projectOpts()));

    expect(compiled).toContain('First body.');
    expect(compiled.match(/^# One$/gm)).toHaveLength(1);
  });
...
describe('ExportEngine.compileContent from the manuscript zone (#232)', () => {
  it('compiles the zone depth-first in binder order without reading _binder.json', async () => {
    ...
    // A stale legacy binder pointing at ghosts must not influence the compile
    // — since #233 nothing outside migration/restore reads it at all
    files.files.set('Projects/My Book/_binder.json',
      '{"version":"2.0","projectId":"project-1","items":[{"id":"g","title":"Ghost","filePath":"Projects/My Book/Ghost.md","type":"chapter","order":1,"status":"draft","includeInExport":true}]}');
  ...
  it('never compiles the Research zone and excludes binder-compile: false documents', async () => {
    ...
    // The zone boundary is the compile boundary — Research never compiles
...
describe('ExportEngine.export markdown end to end', () => {
  it('writes the compiled markdown into the project Exports folder', async () => {
    const { engine, files } = await makeFsWorld(true);

    const outputPath = await engine.export(projectOpts());

    expect(outputPath).toMatch(/^Projects\/My Book\/Exports\/My Book-.*\.md$/);
    const written = files.files.get(outputPath);
    expect(written).toContain('First body.');
